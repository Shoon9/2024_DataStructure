...
#3
platinumQueue = ListQueue()
goldQueue = ListQueue()
silverQueue = ListQueue()
...
class Producer:
    ...
    def get_item(self):
        if self.pos < len(self.items):
            ...
            if item[0] == "3":
                platinumQueue.enqueue(item)
            elif item[0] == "2":
                goldQueue.enqueue(item)
            elif item[0] == "1":
                silverQueue.enqueue(item)
            self.pos += 1
            return item
        else:
            return None
...
class Consumer:
    ...
    def run(self):
        while self.__alive:
            time.sleep(1)  # 주어진 시간만큼 대기
            if not platinumQueue.isEmpty():
                boarditem = platinumQueue.dequeue()
                print("Boarding:", boarditem)
            elif not goldQueue.isEmpty():
                boarditem = goldQueue.dequeue()
                print("Boarding:", boarditem)
            elif not silverQueue.isEmpty():
                boarditem = silverQueue.dequeue()
                print("Boarding:", boarditem)
