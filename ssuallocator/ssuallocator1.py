...
class Allocator:
    def __init__(self):
        ...
        self.allocated_space = CircularDoublyLinkedList()  # 할당된 공간을 관리하는 양방향 순환 연결 리스트
    ...
    def free(self, id):
        # 메모리 해제 요청을 처리
        for idx, item in enumerate(self.allocated_space):
            if item[0] == id:
                self.allocated_space.remove(item)
                self.free_space.insert(item[1])
                self.used_memory -= item[1]
                break
