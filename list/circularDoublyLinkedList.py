...
class CircularDoublyLinkedList:
    ...
    def pop(self, *args):
        # 가변 파라미터, 인자가 없거나 -1이면 마지막 원소로 처리하기 위함. 파인썬 리스트 규칙 만족
        if self.isEmpty():
            return None
        # 인덱스 i 결정
    ...
    def get(self, *args):
        # 가변 파라미터. 인자가 없거나 -1이면 마지막 원소로 처리하기 위함. 파라미터 리스트 규칙 만족
        if self.isEmpty(): return None
        # 인덱스 i 결정
        if len(args) != 0:  # pop(k)과 같이 인자가 있으면 i = k 설정
            i = args[0]
        if len(args) == 0 or i == -1:  # pop()에 인자가 없거나 pop(-1)이면 i에 맨 끝 인덱스 할당
            i = self.__numItems - 1
        # i번 원소 리턴
    ...
    def _findNode(self, x) -> BidirectNode:
    ...
    def getNode(self, i: int) -> BidirectNode:
        curr = self.__head  # 다음 노드, index: -1
